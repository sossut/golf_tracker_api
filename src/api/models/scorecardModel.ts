...
const getScorecardsByUserId = async (userId: number): Promise<Scorecard[]> => {
  const [rows] = await promisePool.execute<GetScorecard[]>(
    `SELECT scorecard_id, user_id, tee_id, scorecard_date, created_at, type_of_round, COUNT(*) AS rounds_played
    FROM scorecards
    WHERE user_id = ?
